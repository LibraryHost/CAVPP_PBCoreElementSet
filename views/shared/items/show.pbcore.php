...
<?php foreach (metadata($item, array('CAVPP_PBCore', 'Date Broadcast'), array('all'=>true)) as $datebroadcast) { ?>
	<pbcoreAssetDate dateType="Broadcast"><?php echo html_escape($datebroadcast); ?></pbcoreAssetDate>
<?php } ?>
<?php foreach (metadata($item, array('CAVPP_PBCore', 'Date Created'), array('all'=>true)) as $datecreated) { ?>
	<pbcoreAssetDate dateType="Created"><?php echo html_escape($datecreated); ?></pbcoreAssetDate>
<?php } ?>
<?php foreach (metadata($item, array('CAVPP_PBCore', 'Identifier'), array('all'=>true)) as $identifier) { ?>
...
<?php } ?>
<?php foreach (metadata($item, array('CAVPP_PBCore', 'Title'), array('all'=>true)) as $title) { ?>
	<pbcoreTitle><?php echo html_escape($title); ?></pbcoreTitle>
<?php } ?>
<?php foreach (metadata($item, array('CAVPP_PBCore', 'Episode Title'), array('all'=>true)) as $episodetitle) { ?>
	<pbcoreTitle titleType="Episode"><?php echo html_escape($episodetitle); ?></pbcoreTitle>
<?php } ?>
<?php foreach (metadata($item, array('CAVPP_PBCore', 'Series Title'), array('all'=>true)) as $seriestitle) { ?>
...
<?php } ?>
<?php foreach (metadata($item, array('CAVPP_PBCore', 'Description'), array('all'=>true)) as $description) { ?>
	<pbcoreDescription><?php echo html_escape($description); ?></pbcoreDescription>
<?php } ?>
<?php foreach (metadata($item, array('CAVPP_PBCore', 'Transcription'), array('all'=>true)) as $transcription) { ?>
...
<?php foreach (metadata($item, array('CAVPP_PBCore', 'Creator'), array('all'=>true)) as $creator) { ?>
	<pbcoreCreator>
		<creator><?php echo html_escape($creator); ?></creator>
		<creatorRole>Creator</creatorRole>
	</pbcoreCreator>
<?php } ?>
<?php foreach (metadata($item, array('CAVPP_PBCore', 'Interviewee'), array('all'=>true)) as $interviewee) { ?>
	<pbcoreContributor>
		<contributor><?php echo html_escape($interviewee); ?></contributor>
		<contributorRole><?php echo "Interviewee"; ?></contributorRole>
	</pbcoreContributor>
<?php } ?>
<?php foreach (metadata($item, array('CAVPP_PBCore', 'Interviewer'), array('all'=>true)) as $interviewer) { ?>
	<pbcoreContributor>
		<contributor><?php echo html_escape($interviewer); ?></contributor>
		<contributorRole><?php echo "Interviewer"; ?></contributorRole>
	</pbcoreContributor>
<?php } ?>
<?php foreach (metadata($item, array('CAVPP_PBCore', 'Host'), array('all'=>true)) as $host) { ?>
	<pbcoreContributor>
		<contributor><?php echo html_escape($host); ?></contributor>
		<contributorRole><?php echo "Host"; ?></contributorRole>
	</pbcoreContributor>
<?php } ?>
<?php foreach (metadata($item, array('CAVPP_PBCore', 'Rights'), array('all'=>true)) as $rights) { ?>
	<pbcoreRightsSummary>
...
<?php if (count(metadata($item, array('CAVPP_PBCore', 'Digital Format'), array('all' => true)))) { ?>
		<instantiationDigital><?php echo metadata($item, array('CAVPP_PBCore', 'Digital Format')); ?></instantiationDigital>
<?php } ?>
<?php if (count(metadata($item, array('CAVPP_PBCore', 'Digital Location'), array('all' => true)))) { ?>
		<instantiationLocation><?php echo metadata($item, array('CAVPP_PBCore', 'Digital Location')); ?></instantiationLocation>
<?php } ?>
<?php if (count(metadata($item, array('CAVPP_PBCore', 'Digital Location'), array('all' => true))) == 0 ) { ?>
		<instantiationLocation>No Location Available</instantiationLocation>
<?php } ?>
<?php if (count(metadata($item, array('CAVPP_PBCore', 'Duration'), array('all' => true)))) { ?>
		<instantiationDuration><?php echo metadata($item, array('CAVPP_PBCore', 'Duration')); ?></instantiationDuration>
<?php } ?>
...
<?php foreach (metadata($item, array('CAVPP_PBCore', 'Physical Location'), array('all'=>true)) as $physical_location) { ?>
	<pbcoreInstantiation>
<?php foreach (metadata($item, array('CAVPP_PBCore', 'Identifier'), array('all'=>true)) as $identifier) { ?>
		<instantiationIdentifier source="Omeka"><?php echo html_escape($identifier); ?></instantiationIdentifier>
<?php } ?>
		<instantiationPhysical><?php echo metadata($item, array('CAVPP_PBCore', 'Physical Format')); ?></instantiationPhysical>
		<instantiationLocation><?php echo metadata($item, array('CAVPP_PBCore', 'Physical Location')); ?></instantiationLocation>
	</pbcoreInstantiation>
<?php } ?>
<?php foreach (metadata($item, array('CAVPP_PBCore', 'Notes'), array('all'=>true)) as $notes) { ?>
	<pbcoreAnnotation annotationType="Notes"><?php echo html_escape($notes); ?></pbcoreAnnotation>
<?php } ?>
<?php foreach (metadata($item, array('CAVPP_PBCore', 'Music/Sound Used'), array('all'=>true)) as $music_sound_used) { ?>
	<pbcoreAnnotation annotationType="MusicUsed"><?php echo html_escape($music_sound_used); ?></pbcoreAnnotation>
<?php } ?>
<?php foreach (metadata($item, array('CAVPP_PBCore', 'Date Peg'), array('all'=>true)) as $date_peg) { ?>
